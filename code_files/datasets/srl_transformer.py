...
class SRLDataset_transformer():
    def __init__(self, lang_data_path:str, split_predicates = False):
        ...
        # adding roles
        self.id_to_roles = [self.nolabel_value] + ['agent', 'asset', 'attribute', 'beneficiary', 'cause', 'co-agent', 'co-patient', 
        ...
        self.id_to_predicates = [self.nolabel_value] + ['ABSORB', 'ABSTAIN_AVOID_REFRAIN', 'ACCOMPANY', 'ACCUSE', 'ACHIEVE', 'ADD', 'ADJUST_CORRECT', 'AFFECT', 
    ...
    def create_collate_fn(self):
        def collate_fn(batch):
            ...
            if 'predicates' in batch[0].keys():
                batch_formatted['predicates'] = [sample['predicates'] for sample in batch]
                batch_formatted['predicates_positions'] = [[1 if s != self.predicates_pad else 0 for s in predicates] for predicates in batch_formatted['predicates']]
            ...
            if all(p == '_' for p in sample['predicates']): # if there won't be any roles for this sentence:
                ...
                for i, predicate in enumerate(sample['predicates']):
                    ...
                    if 'roles' in sample_copy:
                        try:
                            roles = sample_copy['roles'][int(i)]
                        except:
                            roles = sample_copy['roles'][str(i)]
